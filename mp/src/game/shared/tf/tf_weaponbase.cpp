...
bool CTFWeaponBase::Holster( CBaseCombatWeapon *pSwitchingTo )
{
	...
	if (pSwitchingTo) {
		...
		float flOriginalPrimaryAttack = pSwitchingTo->m_flNextPrimaryAttack;
		...
		if (bHolster)
		{
			...
			float flHolsterTime = m_pWeaponInfo->GetWeaponData(m_iWeaponMode).m_iHolsterSpeed;
			pSwitchingTo->m_flNextPrimaryAttack = max(flOriginalPrimaryAttack, gpGlobals->curtime + flHolsterTime);
...
bool CTFWeaponBase::Deploy( void )
{
	...
	if ( bDeploy )
	{
		...
		// Don't override primary attacks that are already further out than this. This prevents
		// people exploiting weapon switches to allow weapons to fire faster.
		float flDeployTime = m_pWeaponInfo->GetWeaponData(m_iWeaponMode).m_iDeploySpeed;
